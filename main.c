...
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height) {
    ...
    // #pragma omp parallel for collapse(2) num_threads(omp_get_max_threads()*2) esto es para utilizar el doble de hilos en los for anidados
...
int calcularSumaPixeles(int *imagen, int width, int height) {
    ...
    #pragma omp parallel for reduction(+:suma)  // Paralelizar y reducir para calcular la suma de los píxeles
